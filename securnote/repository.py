...
from .models import User, AuthData, ZKData, Certificate, Challenge, RevocationEntry, Note
from .exceptions import StorageError, FileCorruptedError, UserNotFoundError
...
class ChallengeRepository(BaseRepository):
    ...
    def save_challenge(self, challenge: Challenge) -> None:
        """Save challenge data."""
        challenge_file = os.path.join(self.data_dir, f"challenge_{challenge.challenge_id}.json")
...
class RevocationRepository(BaseRepository):
    ...
    def is_revoked(self, cert_id: str) -> bool:
        ...
        crl_data = self._load_crl()
        return any(entry['cert_id'] == cert_id for entry in crl_data['revoked_certificates'])
