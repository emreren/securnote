...
def revoke_user_certificate_admin(
    username: str, reason: str = "admin_revocation", admin_user=Depends(verify_admin)
):
    """Revoke user's certificate (Admin only)."""
    logger.warning(
        f"Admin {admin_user} attempting to revoke certificate for user: {username}, reason: {reason}"
    )

    success = app_instance.revoke_user_certificate(username, reason)
    if not success:
        logger.error(
            f"Certificate revocation failed for user: {username} by admin: {admin_user}"
        )
        raise HTTPException(status_code=404, detail="User or certificate not found")

    logger.critical(
        f"Certificate REVOKED for user: {username} by admin: {admin_user}, reason: {reason}"
    )
